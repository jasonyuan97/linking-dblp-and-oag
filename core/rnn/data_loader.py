...
class PairTextDataset(object):
    ...
    def __init__(self, file_dir, seed, shuffle, max_sequence_length, max_key_sequence_length, batch_size, multiple):
        ...
        self.test_data = json.load(codecs.open(join(settings.VENUE_DATA_DIR, 'test.txt'), 'r', 'utf-8'))
    ...
    def get_test_loader(self):
        mag = self.tokenizer.texts_to_sequences([p[1] for p in self.test_data])
        aminer = self.tokenizer.texts_to_sequences([p[2] for p in self.test_data])
        labels = []
        length_mag = []
        length_aminer = []
        keyword_mag = []
        ...
        jaccard = []
        inverse_pairs = []
        for i, pair in enumerate(self.test_data):
            len_mag, len_aminer, key_mag, key_aminer, jacc, inverse = self.preprocess(mag[i], aminer[i])
            labels.append(pair[0])
            length_mag.append(len_mag)
            length_aminer.append(len_aminer)
            keyword_mag.append(key_mag)
            keyword_aminer.append(key_aminer)
            jaccard.append([np.float32(jacc)] * (self.multiple * 2))
            inverse_pairs.append([np.float32(inverse)] * self.multiple)

        mag = pad_sequences(mag, maxlen=self.msl)
        aminer = pad_sequences(aminer, maxlen=self.msl)

        labels, mag, aminer, length_mag, length_aminer, keyword_mag, keyword_aminer, jaccard, inverse_pairs = np.array(
            labels), np.array(mag), np.array(aminer), np.array(length_mag), np.array(
            length_aminer), np.array(keyword_mag), np.array(keyword_aminer), np.array(
            jaccard), np.array(inverse_pairs)

        test = {'mag': mag, 'aminer': aminer, 'keyword_mag': keyword_mag, 'keyword_aminer': keyword_aminer, 'jaccard': jaccard,
                 'inverse': inverse_pairs, 'labels': labels}

        return DataLoader(self.batch_size, test)
